...
def manage_assets():
    if request.method == "GET":
        return jsonify({"success": True, "assets": database.get_assets()})
    ...
    try:
        database.add_asset(data["name"], data["type"], float(data["value"]), float(data["monthly_yield"]))
...
def update_delete_asset(asset_id):
    ...
    data = request.json
    try:
        database.update_asset(asset_id, data["name"], data["type"], float(data["value"]), float(data["monthly_yield"]))
...
def manage_liabilities():
    if request.method == "GET":
        return jsonify({"success": True, "liabilities": database.get_liabilities()})
    ...
    try:
        database.add_liability(data["name"], data["type"], float(data["total_amount"]), float(data["remaining_amount"]), float(data["monthly_cost"]))
...
def update_delete_liability(lib_id):
    if request.method == "DELETE":
        try:
            database.delete_liability(lib_id)
    ...
    data = request.json
    try:
        database.update_liability(lib_id, data["name"], data["type"], float(data["total_amount"]), float(data["remaining_amount"]), float(data["monthly_cost"]))
...
def manage_transactions():
    if request.method == "GET":
        return jsonify({"success": True, "transactions": database.get_transactions()})
    
    data = request.json
    try:
        database.add_transaction(data["description"], data["type"], float(data["amount"]), data["date"])
...
def webhook_shopify():
    ...
    try:
        ...
        if store_asset:
            database.update_asset(store_asset["id"], store_asset["name"], "Commerce", float(data["value"]), float(data["monthly_profit"]))
            action = "updated"
        else:
            database.add_asset(f"Shopify Store - {data['store_name']}", "Commerce", float(data["value"]), float(data["monthly_profit"]))
            action = "created"
            
        # Add notification transaction
        database.add_transaction(f"Webhook Update: Shopify Store '{data['store_name']}' synced", "asset_yield", float(data["monthly_profit"]), "Today")
...
def webhook_tiktok():
    ...
    try:
        ...
        if store_asset:
            ...
            database.add_asset(f"TikTok Shop - {data['store_name']}", "Commerce", float(data["value"]), float(data["monthly_profit"]))
            action = "created"
            
        # Add notification transaction
        database.add_transaction(f"Webhook Update: TikTok Shop '{data['store_name']}' synced", "asset_yield", float(data["monthly_profit"]), "Today")
...
def toggle_user_premium():
    ...
    try:
        database.set_premium_status(status)
...
def update_settings():
    ...
    try:
        database.update_user_currency(currency)
...
def manage_savings_goals():
    if request.method == "GET":
        try:
            return jsonify({"success": True, "savings_goals": database.get_savings_goals()})
        except Exception as e:
            return jsonify({"success": False, "error": str(e)}), 500
    ...
    try:
        database.add_savings_goal(name, float(target_amount), float(current_amount), target_date)
...
def update_delete_savings_goal(goal_id):
    ...
    try:
        database.update_savings_goal(goal_id, name, float(target_amount), float(current_amount), target_date)
...
def webhook_revenuecat():
    # RevenueCat sends authorization header or secret, we can check it
    # We parse the event structure
    data = request.json or {}
    ...
    try:
        if event_type in ["INITIAL_PURCHASE", "RENEWAL", "SUBSCRIBE"]:
            # Set user premium
            database.set_premium_status(1, subscriber_id=app_user_id)
            database.add_transaction(f"Abonnement Premium activé via RevenueCat", "asset_yield", 0.0, "Today")
        elif event_type in ["CANCELLATION", "EXPIRATION"]:
            # Cancel user premium
            database.set_premium_status(0)
            database.add_transaction(f"Abonnement Premium expiré via RevenueCat", "liability_payment", 0.0, "Today")
...
def coach_chat():
    ...
    profile = database.get_user_profile()
    if profile.get("premium_status", 0) == 0:
        blocked_msg = {
            ...
            "pt": "⚠️ O acesso ao Coach Philia Vault e às auditorias mensais é reservado para membros Premium. Por favor, assine para continuar.",
    ...
    lang_directives = {
        ...
        "pt": "Expresse-se em português. Seja motivador, conciso e profissional.",
