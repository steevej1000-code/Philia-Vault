...
def test_v3_features():
    ...
    database.add_savings_goal("Apport Immobilier", 50000.0, 12000.0, "Dec 2026")
    ...
    res = requests.get(f"{base_url}/api/savings_goals")
    ...
    # A. Test initial user currency (should be USD)
    print("Testing initial user profile settings...")
    res = requests.get(f"{base_url}/api/user")
    assert res.status_code == 200
    profile = res.json()["user"]
    print(f"Current currency setting: {profile.get('currency')} (Expected: USD)")
    assert profile.get("currency") == "USD"
    
    # B. Update currency via API
    print("Updating user settings currency to EUR...")
    res = requests.post(f"{base_url}/api/user/settings", json={"currency": "EUR"})
    assert res.status_code == 200
    ...
    res = requests.get(f"{base_url}/api/user")
    profile = res.json()["user"]
    print(f"New currency setting: {profile.get('currency')} (Expected: EUR)")
