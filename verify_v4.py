...
def test_v4_features():
    ...
    headers = {"Authorization": f"Bearer {generate_jwt('alex@philiavault.com')}"}
    ...
    try:
        ...
        res = requests.post(f"{base_url}/api/user/premium", json={"premium_status": 0}, headers=headers)
        assert res.status_code == 200

        print("Making non-premium request in French...")
        res_fr = requests.post(f"{base_url}/api/coach/chat", json={"lang": "fr", "message": "Bonjour"}, headers=headers)
        ...
        res_de = requests.post(f"{base_url}/api/coach/chat", json={"lang": "de", "message": "Hallo"}, headers=headers)
        ...
        res_en = requests.post(f"{base_url}/api/coach/chat", json={"lang": "en", "message": "Hello"}, headers=headers)
        ...
        res = requests.post(f"{base_url}/api/user/premium", json={"premium_status": 1}, headers=headers)
        assert res.status_code == 200

        print("Making premium request in German...")
        res_de_prem = requests.post(f"{base_url}/api/coach/chat", json={"lang": "de", "message": "Bonjour"}, headers=headers)
        ...
        requests.post(f"{base_url}/api/user/premium", json={"premium_status": original_premium}, headers=headers)
