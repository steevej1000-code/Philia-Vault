...
import {
  View, Text, StyleSheet, ScrollView, TextInput, TouchableOpacity, Animated,
} from 'react-native';
...
import { IconTrendUp, IconClock, IconWallet, IconTarget, IconPlus } from './icons/Icons';
...
function useCountUp(target: number, duration = 600) {
  ...
  useEffect(() => {
    ...
    const listener = anim.addListener(({ value }) => setDisplay(value));
...
/**
 * Reusable simulator body (gauge, parameters, results).
 * Used both by the standalone simulator screen and the Dashboard modal.
 */
export function SimulatorContent() {
  ...
  const annualIncome = yieldNum * 12;

  const animatedRoi = useCountUp(roi);
  const animatedNetCashflow = useCountUp(netCashflow);
  const animatedAnnualIncome = useCountUp(annualIncome);
  const animatedBreakEven = useCountUp(isFinite(breakEvenMonths) ? breakEvenMonths : 0);

  const handleAddAsset = () => {
    router.push({
      pathname: '/(tabs)/assets',
      params: {
        prefillName: investmentName,
        prefillValue: String(capNum),
        prefillYield: String(yieldNum),
        openAdd: '1',
      },
    } as any);
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
      ...
      <GlassCard>
        ...
        <View style={styles.resultsGrid}>
          <View style={[styles.resultCard, { backgroundColor: 'rgba(204,255,0,0.08)', borderColor: 'rgba(204,255,0,0.2)' }]}>
            ...
            <Text style={styles.resultValue} numberOfLines={1}>{animatedRoi.toFixed(1)}%</Text>
          ...
          <View style={[styles.resultCard, { backgroundColor: 'rgba(6,182,212,0.08)', borderColor: 'rgba(6,182,212,0.2)' }]}>
            ...
            <Text style={styles.resultValue} numberOfLines={1}>
              {isFinite(breakEvenMonths) ? `${Math.round(animatedBreakEven)}m` : '∞'}
            </Text>
          ...
          <View style={[styles.resultCard, { backgroundColor: netCashflow >= 0 ? 'rgba(204,255,0,0.08)' : 'rgba(239,68,68,0.08)', borderColor: netCashflow >= 0 ? 'rgba(204,255,0,0.2)' : 'rgba(239,68,68,0.2)' }]}>
            ...
            <Text style={[styles.resultValue, { color: netCashflow >= 0 ? COLORS.primary : COLORS.error }]} numberOfLines={1}>
              {fmtCurrency(animatedNetCashflow)}
            </Text>
          ...
          <View style={[styles.resultCard, { backgroundColor: 'rgba(139,92,246,0.08)', borderColor: 'rgba(139,92,246,0.2)' }]}>
            ...
            <Text style={[styles.resultValue, { color: COLORS.tertiary }]} numberOfLines={1}>
              {fmtCurrency(animatedAnnualIncome)}
            </Text>
      ...
      {/* CTA */}
      <TouchableOpacity style={styles.ctaButton} onPress={handleAddAsset} activeOpacity={0.85}>
        <IconPlus size={20} color={COLORS.onPrimary} />
        <Text style={styles.ctaButtonText}>{t('add_this_asset')}</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  ctaButton: {
    ...
    shadowColor: COLORS.primary, shadowOpacity: 0.4, shadowRadius: 16, shadowOffset: { width: 0, height: 6 }, elevation: 6,
  },
  ctaButtonText: { fontSize: 15, fontWeight: '800', color: COLORS.onPrimary, letterSpacing: 0.3 },
});
