...
TEST_USER_EMAIL = "test_alex_user@philiavault.com"
...
def test_iap_flow():
    ...
    try:
        ...
        res = requests.post(f"{base_url}/api/coach/chat", json={"message": "Audit financier"}, headers=headers)
        assert res.status_code == 200
        chat_data = res.json()
        ...
        # 3. Simulate RevenueCat Purchase Webhook (no JWT needed: RevenueCat
        # identifies the user via app_user_id, not a session token)
        print("Simulating RevenueCat INITIAL_PURCHASE webhook...")
        payload = {
            "event": {
                "type": "INITIAL_PURCHASE",
                "app_user_id": TEST_USER_EMAIL
            }
        }
        ...
        res = requests.post(f"{base_url}/api/coach/chat", json={"message": "audit"}, headers=headers)
        assert res.status_code == 200
        chat_data = res.json()
